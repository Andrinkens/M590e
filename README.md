...
## M590e_bribge.ino
скетч, позволяющий превратить Arduino nano (или uno) в преобразователь USB <> UART. Это позволит напрямую из монитора порта исать в модуль "AT" комманды. Модуль должен быть предварительно настроен на скорость 9600 бод. Если же скорость отличается, её можно подобрать, поменяв в строке:
```c++
